...
exports.updateIndexes = function (options, callback) {
  ...
  indexes.forEach(function (index) {
    ...
    if(operation === 'add' && data.hasOwnProperty(key)) {
      ...
      if(sort) queue.push(addToSortedSet(dbKey, id, 1));
      else queue.push(addToSet(dbKey, id));
    }
    ...
    if((operation === 'add' && prevData && prevData[key] !== data[key]) ||
      ...
      val = prevData[key];
      dbKey = getKey(db, baseKey, key, val);
      if(sort) queue.push(removeFromSortedSet(dbKey, id, 1));
      else queue.push(removeFromSet(dbKey, id));
    }
  });
